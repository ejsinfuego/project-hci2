...
if (isset($_GET['id'])) {
    ...
    function validate($data)
    {
        ...
        $data = htmlspecialchars($data);
        return $data;
        /* Kahit naman ano ilagay ko dito di mo naman mababasa kase di ka naman tumitingin sa source code */
    }
    ...
    if (empty($firstName)) {
        header("Location: ../update.php?id=$id&error=First Name is required");}
            ...
            $result = mysqli_query($conn, $sql);
            if ($result) {
                header("Location: residents.php?success=successfully updated");
            } else {
